const TWILIO_KEYS = {
  accountSid: "ACae508fed97e39b6bbd7343f378cd03bf",
  authToken: "0086395111c172424c5184ce798b9523",
  fromNumber: "+17372583478",
  toNumber: "+19544657538"
};

const LOW_DOSE_SMS_THRESHOLD = 3;

const ALLOWED_USERS = [
  { email: "alberto@inventario.com", password: "hola1234" },
  { email: "marol@inventario.com", password: "hola1234" },
  { email: "doctor@clinic.com", password: "Doctor9012!" }
];
...
function doLogin() {
  ...
  const match = ALLOWED_USERS.find(user => user.email === email && user.password === password);
  ...
  currentUser = match.email;
  sessionStorage.setItem("vw_user", currentUser);
...
function initApp() {
  ...
  const savedUser = sessionStorage.getItem("vw_user");
  if (savedUser) {
...
function loadJars() {
  try {
    const saved = localStorage.getItem("vw_jars");
    jars = saved ? JSON.parse(saved) : [...DEFAULT_JARS];
  } catch {
    jars = [...DEFAULT_JARS];
  }
  saveJars();
...
function saveJars() {
  localStorage.setItem("vw_jars", JSON.stringify(jars));
}
